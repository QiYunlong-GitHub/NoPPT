...
export default function PreviewPage() {
    ...
    const goToSlide = (newIndex: number, direction: 'next' | 'prev') => {
        if (newIndex === currentIndex || isTransitioning || !presentation)
            return;
        if (transition === 'none') {
    ...
    return (<div className="min-h-screen bg-slate-900 flex items-center justify-center relative overflow-hidden" style={{ perspective: '2000px' }}>
      <div className="w-full max-w-6xl mx-4 flex justify-center">
        <div className="relative" style={{
            aspectRatio: `${slideWidth} / ${slideHeight}`,
            width: '100%',
        }}>
          {isTransitioning && prevIndex !== currentIndex && (<div className="absolute inset-0 bg-white rounded-lg shadow-2xl overflow-hidden" style={{
          ...
          <div className="absolute inset-0 bg-white rounded-lg shadow-2xl overflow-hidden" style={{
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 z-50 opacity-60 hover:opacity-100 transition-opacity duration-300">
        <button onClick={() => goToSlide(Math.max(currentIndex - 1, 0), 'prev')} disabled={currentIndex === 0 || isTransitioning} className="p-3 bg-black/40 backdrop-blur-sm text-white rounded-full hover:bg-black/60 disabled:opacity-30 transition-all">
        ...
        <button onClick={() => goToSlide(Math.min(currentIndex + 1, presentation.slides.length - 1), 'next')} disabled={currentIndex === presentation.slides.length - 1 || isTransitioning} className="p-3 bg-black/40 backdrop-blur-sm text-white rounded-full hover:bg-black/60 disabled:opacity-30 transition-all">
      ...
      <div className="absolute top-6 right-6 flex items-center gap-2 z-50 opacity-60 hover:opacity-100 transition-opacity duration-300">
        <button onClick={() => window.history.back()} className="p-2 bg-black/40 backdrop-blur-sm text-white rounded-lg hover:bg-black/60 transition-all">
          <X className="w-5 h-5"/>
        </button>
      </div>
      ...
      <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-50 opacity-60 hover:opacity-100 transition-opacity duration-300">
        {presentation.slides.map((_, i) => (<button key={i} onClick={() => goToSlide(i, i > currentIndex ? 'next' : 'prev')} className={`w-2 h-2 rounded-full transition-all ${i === currentIndex ? 'bg-white w-3' : 'bg-white/30 hover:bg-white/50'}`}/>))}
      </div>
    </div>);
}
