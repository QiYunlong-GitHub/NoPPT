...
import { FileText, Trash2, Edit3, ArrowLeft, Settings, Layout, Clock, MoreVertical, } from 'lucide-react';
...
export default function PresentationsPage() {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
      ...
      <header className="border-b border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title={t("返回首页")}>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-xl text-slate-800 dark:text-slate-100">{t('所有演示')}</span>
            </div>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        {presentations.length === 0 ? (<div className="text-center py-20">
            <div className="w-20 h-20 mx-auto mb-6 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center">
              <FileText className="w-10 h-10 text-slate-400"/>
            </div>
            <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-2">{t('还没有演示文稿')}</h3>
            <p className="text-slate-500 dark:text-slate-400 mb-6">{t('创建你的第一个演示文稿吧')}</p>
            <button onClick={() => navigate('/')} className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-slate-700 text-white rounded-lg hover:bg-slate-800 dark:hover:bg-slate-600 transition-colors font-medium">
              {t('返回首页创建')}
            </button>
          </div>) : (<div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            {/* Table header */}
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {presentations.map((pres, index) => {
                ...
                return (<div key={pres.id} className="px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                  <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-7 flex items-center gap-3 min-w-0">
                      ...
                      {editingId === pres.id ? (<input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} onBlur={() => handleSaveRename(pres.id)} onKeyDown={(e) => {
                            ...
                            if (e.key === 'Escape')
                                setEditingId(null);
                        }} autoFocus className="flex-1 bg-slate-50 dark:bg-slate-800 border border-blue-400 rounded px-2 py-1.5 text-sm text-slate-700 dark:text-slate-300 outline-none min-w-0" onClick={(e) => e.stopPropagation()}/>) : (<button onClick={() => openPresentation(pres.id)} className="flex-1 text-left font-medium text-slate-800 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 truncate min-w-0" title={pres.title}>
                          {pres.title}
                        </button>)}
                    </div>
                    ...
                    <div className="col-span-2 flex items-center justify-end gap-2 relative">
                      <button onClick={() => openPresentation(pres.id)} className="px-3 py-1.5 text-sm text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md transition-colors font-medium">
                        {t('编辑')}
                      </button>
                      ...
                      <div className="relative">
                        ...
                        {menuOpenId === pres.id && (<div className="absolute right-0 top-full mt-1 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 py-1 z-20 min-w-[120px]" onClick={(e) => e.stopPropagation()}>
