...
import { Plus, FileText, Settings, Sparkles, Layout, Zap, Lock, Trash2, Edit3, MoreVertical, ChevronRight, RefreshCw, ExternalLink, Copy } from 'lucide-react';
...
export default function HomePage() {
    ...
    useEffect(() => {
        ...
        if (!draftId)
            return;
        if (handledDraftRef.current === draftId)
            return;
        // sessionStorage 兜底：刷新/回退回到首页时组件重新挂载，ref 会丢，避免重复建演示。
        if (sessionStorage.getItem(`noppt:handled-draft:${draftId}`))
            return;
        ...
        void (async () => {
            ...
            try {
                const draft = await draftApi.get(draftId, params.get('t') ?? '', params.get('u') ?? '', params.get('token') ?? '');
                const { editorSettings } = useSettingsStore.getState();
                const pres = await createPresentation(draft.topic?.trim() || t("AI生成演示"), editorSettings?.defaultSlideWidth, editorSettings?.defaultSlideHeight);
                ...
                const message = err instanceof DraftFetchError
                    ? ({
                        expired: t('该生成链接已过期，请让助手重新生成一条'),
                        not_found: t('未找到该生成草稿，请让助手重新生成一条链接'),
                        forbidden: t('生成链接无效或无访问权限'),
                        unknown: err.message,
                    } as Record<DraftFetchError['kind'], string>)[err.kind]
    ...
    const handleCreateNew = async () => {
        ...
        setIsCreating(true);
        try {
            const pres = await createPresentation(t("新演示文稿"), editorSettings.defaultSlideWidth, editorSettings.defaultSlideHeight);
    ...
    const handleCreateAI = async () => {
        ...
        setIsCreating(true);
        try {
            const pres = await createPresentation(t("AI生成演示"), editorSettings.defaultSlideWidth, editorSettings.defaultSlideHeight);
            navigate(`/editor/${pres.id}?ai=1`);
        }
        finally {
            setIsCreating(false);
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              <button onClick={() => navigate('/settings')} className="p-2.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
      ...
      <section className="max-w-4xl mx-auto px-6 py-16 text-center">
        ...
        <p className="text-lg text-slate-600 dark:text-slate-400 mb-8 max-w-2xl mx-auto">
          {t('开源、高性能、可私有化部署的卡片式演示平台。一句话生成完整演示，拖拽微调，一键导出。')}
        </p>
        <div className="flex items-center justify-center gap-4">
          <button onClick={handleCreateNew} disabled={isCreating} className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 dark:bg-slate-700 text-white rounded-xl hover:bg-slate-800 dark:hover:bg-slate-600 transition-colors font-medium disabled:opacity-50">
            <Plus className="w-5 h-5"/>
            {t('新建空白演示')}
          </button>
          <button onClick={handleCreateAI} disabled={isCreating} className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:opacity-90 transition-opacity font-medium disabled:opacity-50">
      ...
      {/* Recent */}
      {recentPresentations.length > 0 && (<section className="max-w-6xl mx-auto px-6 pb-16">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white">{t('最近编辑')}</h2>
            <div className="flex items-center gap-2">
              <button onClick={() => loadAllPresentations()} className="inline-flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 font-medium p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" title={t('刷新列表')}>
                <RefreshCw className="w-4 h-4"/>
              </button>
              <button onClick={() => navigate('/presentations')} className="inline-flex items-center gap-1 text-sm text-blue-600 hover:text-blue-700 font-medium">
                {t('所有演示')}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {recentPresentations.map((pres) => (<div key={pres.id} className="group relative text-left">
                ...
                }} className="p-1.5 bg-white/90 dark:bg-slate-800/90 rounded-lg hover:bg-white dark:hover:bg-slate-700 shadow-sm border border-slate-200 dark:border-slate-700 transition-all" title={t('在新标签页打开')}>
                    <ExternalLink className="w-4 h-4 text-slate-600 dark:text-slate-400"/>
                  </button>
                  <button onClick={(e) => {
                    e.stopPropagation();
                    setMenuOpenId(menuOpenId === pres.id ? null : pres.id);
                }} className="p-1.5 bg-white/90 dark:bg-slate-800/90 rounded-lg hover:bg-white dark:hover:bg-slate-700 shadow-sm border border-slate-200 dark:border-slate-700 transition-all" title={t('更多操作')}>
                    <MoreVertical className="w-4 h-4 text-slate-600 dark:text-slate-400"/>
                  </button>
                </div>

                {/* Dropdown menu */}
                {menuOpenId === pres.id && (<div className="absolute top-10 right-2 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 py-1 z-20 min-w-[120px]" onClick={(e) => e.stopPropagation()}>
                    <button onClick={(e) => handleStartRename(pres, e)} className="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 flex items-center gap-2">
                      <Edit3 className="w-4 h-4"/>{t("重命名")}</button>
                    <button onClick={(e) => handleDuplicate(pres.id, e)} className="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 flex items-center gap-2">
                      <Copy className="w-4 h-4"/>{t("复制")}</button>
                    <button onClick={(e) => handleDelete(pres.id, e)} className="w-full px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2">
                      <Trash2 className="w-4 h-4"/>{t("删除")}</button>
                  </div>)}

                {/* Title */}
                {editingId === pres.id ? (<input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} onBlur={() => handleSaveRename(pres.id)} onKeyDown={(e) => {
                        if (e.key === 'Enter')
                            handleSaveRename(pres.id);
                        if (e.key === 'Escape')
                            setEditingId(null);
                    }} autoFocus className="mt-2 text-sm font-medium text-slate-700 dark:text-slate-300 w-full bg-slate-50 dark:bg-slate-800 border border-blue-400 rounded px-2 py-1 outline-none" onClick={(e) => e.stopPropagation()}/>) : (<p className="mt-2 text-sm font-medium text-slate-700 dark:text-slate-300 truncate">
                    {pres.title}
                  </p>)}
