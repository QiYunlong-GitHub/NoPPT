...
const fileOptions: Array<{
    ...
    { value: 'detailed', label: t("详细模式"), desc: t("记录配置参数、请求报文、响应报文、后处理结果") },
...
export default function LogSettings() {
    ...
    const SegmentedControl = ({ value, onChange, options, }: {
      ...
      <div className="inline-flex rounded-xl border border-slate-200 dark:border-slate-600 overflow-hidden bg-slate-50 dark:bg-slate-700/50 p-1">
        {options.map((opt) => {
            ...
            return (<button key={opt.value} type="button" onClick={() => onChange(opt.value)} className={`px-5 py-2 text-sm font-medium rounded-lg transition-all ${active
    ...
    return (<section className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
      ...
      <div className="p-6 space-y-8">
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-white">{t('控制台输出')}</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t('控制浏览器/终端控制台的日志详细程度')}
              </p>
            </div>
          </div>
          <SegmentedControl value={settings.logSettings.consoleVerbosity} onChange={(v) => settings.updateLogSettings({ consoleVerbosity: v })} options={consoleOptions}/>
        </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-white">{t('日志记录（文件）')}</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t('控制写入 ai-log.jsonl 的内容详细程度')}
              </p>
            </div>
          </div>
          <SegmentedControl value={settings.logSettings.fileVerbosity} onChange={(v) => settings.updateLogSettings({ fileVerbosity: v })} options={fileOptions}/>
        </div>

        {/* 使用提示 */}
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-300 mb-1">{t("💡")}{t('使用建议')}</p>
