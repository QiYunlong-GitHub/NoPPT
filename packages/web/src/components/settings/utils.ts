...
export function getDefaultModelPlaceholder(provider: string): string {
  switch (provider) {
    case 'freeai':
      return '例如：qwen7b、qwen-coder、openai/gpt-4o-mini';
    case 'v0':
      return '例如：v0-1.5-md';
    case 'anthropic':
      return '例如：claude-3-5-sonnet-20241022';
    case 'ollama':
      return '例如：llama3.1';
...
export function getBaseUrlHint(provider: string): string {
  switch (provider) {
    case 'freeai':
      return 'Free.ai API 地址，默认为官方地址';
    case 'v0':
      return 'v0 Platform API 地址，默认为官方地址';
    case 'anthropic':
      return 'Anthropic API 地址，默认为官方地址，也可配置兼容代理地址';
    case 'ollama':
