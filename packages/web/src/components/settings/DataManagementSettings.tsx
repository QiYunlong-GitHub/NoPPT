  ...
  const applySettingsFromData = (data: any, preserveApiKeys: boolean = true) => {
    ...
    if (preserveApiKeys && data.imageGeneration?.providers) {
      for (const key of Object.keys(settings.imageGeneration.providers)) {
        currentImageApiKeys[key] =
          settings.imageGeneration.providers[
            key as keyof typeof settings.imageGeneration.providers
          ].apiKey;
      }
    }
    if (data.apiConfig) {
    ...
    if (data.imageGeneration) {
      ...
      if (imgConfig.providers) {
        for (const key of Object.keys(imgConfig.providers)) {
          if (
            preserveApiKeys &&
            (!imgConfig.providers[key].apiKey || imgConfig.providers[key].apiKey === '')
          ) {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (data.presentations && Array.isArray(data.presentations)) {
          ...
          showToast(
            t('演示数据导入成功，共 {n} 个演示文稿', { n: data.presentations.length }),
            'success',
          );
        } else {
          ...
          <p className="text-xs text-slate-500 dark:text-slate-500 mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
            ...
            {settings.configSource === 'file' && (
              <span className="text-green-600 dark:text-green-400 font-medium">
                {t('✓ 配置文件（data/config.json）')}
              </span>
            )}
            {settings.configSource === 'local' && (
              <span className="text-amber-600 dark:text-amber-400 font-medium">
                {t('⚠ 浏览器本地存储（配置文件未找到或无法访问）')}
              </span>
            )}
            {settings.configSource === 'default' && (
