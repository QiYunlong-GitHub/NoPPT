...
import { Plus, Copy, Clipboard, Trash2, GripVertical, Edit3, ExternalLink, Scissors, ImageIcon, ImageOff, Pencil } from 'lucide-react';
...
export default function SlideListPanel() {
    ...
    const handleCopySlides = useCallback(async (slideIds: string[]) => {
        ...
        try {
            try {
                ...
                const singleType = new Blob([JSON.stringify(slides[0])], { type: 'application/x-noppt-slide' });
    ...
    const handleCutSlides = useCallback(async (slideIds: string[]) => {
        if (!presentation)
            return;
    ...
    const handleDeleteSlides = useCallback((slideIds: string[]) => {
        if (!presentation)
            return;
        if (slideIds.length >= presentation.slides.length) {
            showToast(t('至少保留一张幻灯片'), 'error');
    ...
    const handlePasteSlides = useCallback(async (insertIndex?: number) => {
        if (!presentation)
            return;
        try {
    ...
    const checkClipboardHasSlide = async (): Promise<boolean> => {
        try {
            try {
                const clipboardItems = await navigator.clipboard.read();
                for (const item of clipboardItems) {
                    if (item.types.includes('application/x-noppt-slide') || item.types.includes('application/x-noppt-slides')) {
                        return true;
                    }
                }
            }
            ...
            try {
                ...
                if (Array.isArray(parsed) && parsed.length > 0 && parsed[0] && 'id' in parsed[0] && 'html' in parsed[0]) {
                    return true;
                }
            }
            catch {
    ...
    useEffect(() => {
        const handleKeyDown = async (e: KeyboardEvent) => {
            ...
            if (!isTargetInPanel && !isPanelFocused)
                return;
            ...
            if ((e.ctrlKey || e.metaKey) && e.key === 'c') {
                e.preventDefault();
                e.stopPropagation();
                const ids = selectedSlideIds.size > 0 ? Array.from(selectedSlideIds) : (selectedSlideId ? [selectedSlideId] : []);
                if (ids.length > 0) {
    ...
    }, [presentation, selectedSlideId, selectedSlideIds, editingId, handleCopySlides, handleCutSlides, handleDeleteSlides, handlePasteSlides]);
    if (!presentation)
        return null;
    ...
    return (<div className="flex flex-col h-full" ref={panelRef} tabIndex={0}>
      <div className="p-3 border-b border-slate-200 flex items-center justify-between gap-2">
        {isEditingPresTitle ? (<input ref={presTitleInputRef} type="text" value={editPresTitle} onChange={(e) => setEditPresTitle(e.target.value)} onBlur={finishEditingPresTitle} onKeyDown={handlePresTitleKeyDown} className="font-semibold text-slate-800 bg-slate-50 border border-blue-400 rounded-md px-2 py-1 text-sm outline-none focus:ring-2 focus:ring-blue-200 w-full"/>) : (<button onClick={startEditingPresTitle} className="flex items-center gap-1.5 group flex-1 min-w-0 text-left" title={t('点击修改演示名称')}>
        ...
        <button onClick={() => addSlide()} className="p-1 hover:bg-slate-100 rounded transition-colors shrink-0" title={t('新增幻灯片')}>
      ...
      <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
        <span className="text-xs font-medium text-slate-500">
          {t('幻灯片')}
          {selectedCount > 1 && (<span className="ml-2 text-blue-600 font-normal">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1" onContextMenu={handleBlankAreaContextMenu}>
        {presentation.slides.map((slide, index) => (<div key={slide.id} className={cn('group relative rounded-lg overflow-hidden cursor-pointer border-2 transition-all', selectedSlideIds.has(slide.id)
                ? 'border-blue-500 shadow-sm bg-blue-50/30'
                : 'border-transparent hover:border-slate-300', draggedIds?.has(slide.id) && 'opacity-50', dragOverId === slide.id && dragPosition === 'before' && 'border-t-2 border-t-blue-500', dragOverId === slide.id && dragPosition === 'after' && 'border-b-2 border-b-blue-500')} onClick={(e) => handleSlideClick(e, slide.id, index)} onContextMenu={(e) => handleContextMenu(e, slide.id, index)} draggable={!editingId} onDragStart={(e) => handleDragStart(e, slide.id)} onDragOver={(e) => handleDragOver(e, slide.id)} onDragLeave={handleDragLeave} onDrop={(e) => handleDrop(e, slide.id)} onDragEnd={handleDragEnd}>
            ...
            <div ref={index === 0 ? thumbnailMeasureRef : null} className="ml-5 w-full overflow-hidden bg-white" style={{ aspectRatio: `${slideWidth} / ${slideHeight}` }}>
              <div className="origin-top-left bg-white relative" style={{
                width: `${slideWidth}px`,
            ...
            }} className="p-1 bg-white/90 rounded hover:bg-slate-100" title={t('复制')}>
            ...
            }} className="p-1 bg-white/90 rounded hover:bg-red-50" title={t('删除')} disabled={presentation.slides.length <= 1}>
                <Trash2 className="w-3 h-3 text-red-500"/>
              </button>
            </div>

            <div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-[10px] px-2 py-1 truncate">
              {editingId === slide.id ? (<input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} onBlur={handleSaveRename} onKeyDown={(e) => {
                    if (e.key === 'Enter')
                        handleSaveRename();
                    if (e.key === 'Escape')
                        setEditingId(null);
                }} autoFocus onClick={(e) => e.stopPropagation()} className="w-full bg-white/20 border border-white/40 rounded px-1 py-0.5 text-white text-[10px] outline-none"/>) : (slide.title)}
            </div>
          </div>))}
      </div>

      {contextMenu && (<div ref={menuRef} className="fixed bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-50 min-w-[140px]" style={{ left: contextMenu.x, top: contextMenu.y }} onClick={(e) => e.stopPropagation()}>
          <button onClick={handleOpenInNewTab} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <ExternalLink className="w-4 h-4"/>
            {t('在新标签页打开')}
          </button>
          <div className="border-t border-slate-100 my-1"/>
          <button onClick={handleCutClick} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <Scissors className="w-4 h-4"/>
            {t('剪切')}
            <span className="ml-auto text-xs text-slate-400">Ctrl+X</span>
          </button>
          <button onClick={handleCopyClick} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <Copy className="w-4 h-4"/>
            {t('复制')}
            <span className="ml-auto text-xs text-slate-400">Ctrl+C</span>
          </button>
          <button onClick={handlePasteClick} disabled={!canPasteSlide} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            <Clipboard className="w-4 h-4"/>
            {t('粘贴')}
            <span className="ml-auto text-xs text-slate-400">Ctrl+V</span>
          </button>
          <div className="border-t border-slate-100 my-1"/>
          <button onClick={handleSetSingleBg} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <ImageIcon className="w-4 h-4"/>
            {t('设置本页背景图')}
          </button>
          <button onClick={handleRemoveSingleBg} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <ImageOff className="w-4 h-4"/>
            {t('移除本页背景图')}
          </button>
          <div className="border-t border-slate-100 my-1"/>
          <button onClick={handleSetAllBg} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <ImageIcon className="w-4 h-4"/>
            {t('一键设置所有背景图')}
          </button>
          <button onClick={handleRemoveAllBg} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <ImageOff className="w-4 h-4"/>
            {t('一键移除所有背景图')}
          </button>
          <div className="border-t border-slate-100 my-1"/>
          <button onClick={handleStartRename} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <Edit3 className="w-4 h-4"/>
            {t('重命名')}
          </button>
          <button onClick={handleDelete} disabled={presentation.slides.length <= 1 || (selectedSlideIds.size >= presentation.slides.length)} className="w-full px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {blankAreaContextMenu && (<div ref={blankMenuRef} className="fixed bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-50 min-w-[160px]" style={{ left: blankAreaContextMenu.x, top: blankAreaContextMenu.y }} onClick={(e) => e.stopPropagation()}>
          <button onClick={() => handlePasteSlides(presentation.slides.length)} disabled={!canPasteSlide} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            <Clipboard className="w-4 h-4"/>
          ...
          <button onClick={() => { addSlide(); setBlankAreaContextMenu(null); }} className="w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
      ...
      <input ref={bgAllInputRef} type="file" accept="image/*" onChange={handleBgFile} className="hidden"/>
    </div>);
}
