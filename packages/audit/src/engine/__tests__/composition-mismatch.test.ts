...
function makeRva(globalComposition?: 'left-aligned' | 'centered', contentComposition?: 'left-aligned' | 'centered'): any {
  return {
    global: { uploaded: !!globalComposition, visual: globalComposition ? { composition: globalComposition } : undefined },
    byCategory: {
      cover: { uploaded: false },
      content: { uploaded: !!contentComposition, visual: contentComposition ? { composition: contentComposition } : undefined },
...
describe('AuditEngine 构图不符致命检测（regeneration · pres_mtrcm1nx 同批守护）', () => {
  it('正向·单份 global 左对齐参考落到 content + 页面居中 → 触发 layout.composition-mismatch', async () => {
    ...
    const report = await engine.auditPresentation(makePresentation(CENTERED), plan, undefined, { visualAttributes: makeRva('left-aligned') } as any);
  ...
  it('正向·content 显式上传左对齐参考 + 页面居中 → 触发 layout.composition-mismatch', async () => {
    const engine = new AuditEngine(cfg);
    const plan: any = { slides: [{ pageType: 'content-cards' }] };
    const report = await engine.auditPresentation(makePresentation(CENTERED), plan, undefined, { visualAttributes: makeRva(undefined, 'left-aligned') } as any);
    const issue = report.issues.find((i) => i.ruleId === RULE);
    expect(issue).toBeDefined();
    expect(issue?.severity).toBe('error');
  });
  ...
  it('反向·无参考属性 → 不触发', async () => {
    const engine = new AuditEngine(cfg);
    const plan: any = { slides: [{ pageType: 'content-cards' }] };
    const report = await engine.auditPresentation(makePresentation(CENTERED), plan, undefined, { visualAttributes: makeRva() } as any);
  ...
  it('反向·参考构图为居中（非左对齐）+ 页面居中 → 不触发', async () => {
    const engine = new AuditEngine(cfg);
    const plan: any = { slides: [{ pageType: 'content-cards' }] };
    const report = await engine.auditPresentation(makePresentation(CENTERED), plan, undefined, { visualAttributes: makeRva('centered') } as any);
    expect(report.issues.some((i) => i.ruleId === RULE)).toBe(false);
  });
  ...
  it('反向·参考左对齐但页面根容器非居中 → 不触发', async () => {
    const engine = new AuditEngine(cfg);
    const plan: any = { slides: [{ pageType: 'content-cards' }] };
    const report = await engine.auditPresentation(makePresentation(LEFT_ALIGNED), plan, undefined, { visualAttributes: makeRva('left-aligned') } as any);
    expect(report.issues.some((i) => i.ruleId === RULE)).toBe(false);
  });
});
