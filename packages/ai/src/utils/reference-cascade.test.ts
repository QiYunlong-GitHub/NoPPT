...
describe('reference-style-cascade', () => {
  ...
  it('CSS 变量 var() 替换', () => {
    const doc = docOf(
      `<style>:root{--p:#ff4d6d;}.a{color:var(--p);}</style><div class="a">x</div>`,
    );
    const idx = buildCascadeIndex(doc);
    expect(resolveComputedDecl(doc.querySelector('.a') as Element, 'color', '', idx)).toBe(
      '#ff4d6d',
    );
  });
  ...
  it('inline 覆盖级联值', () => {
    ...
    const idx = buildCascadeIndex(doc);
    expect(resolveComputedDecl(doc.querySelector('.a') as Element, 'color', '#999', idx)).toBe(
      '#999',
    );
  });
  ...
  it('后代组合子 .a .b 命中', () => {
    const doc = docOf(
      `<style>.a .b{color:#654321;}</style><div class="a"><span class="b">t</span></div>`,
    );
    const idx = buildCascadeIndex(doc);
    expect(resolveComputedDecl(doc.querySelector('.b') as Element, 'color', '', idx)).toBe(
      '#654321',
    );
  });
  ...
  it('@media 整块被跳过', () => {
    const doc = docOf(
      `<style>@media(max-width:600px){.a{color:#000;}}</style><div class="a">x</div>`,
    );
    const idx = buildCascadeIndex(doc);
    expect(idx.styleRulesCount).toBe(0);
    expect(
      resolveComputedDecl(doc.querySelector('.a') as Element, 'color', '', idx),
    ).toBeUndefined();
  });
});
